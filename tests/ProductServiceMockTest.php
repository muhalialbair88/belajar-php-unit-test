<?php
//menggunakan mock object(createMock) improvement proble di stab(createStab)
// pada mock menggunakan method(name) & expects(InvocationRule) sedangkan stab hanya method(name)
...
class ProductServiceMockTest extends TestCase{
    ...
    {
        $this->repository = $this->createMock(ProductRepository::class); // createStub() ini adalah cara membuat dummy objek pada kelas, jika kelas yang mau kita test berhubungan dengan class lain
    ...
    public function testMock(){
        ...
        $this->repository->expects(self::once())
            ...
            // $result = $this->repository->findById("1"); error karna dipanggilnya 1x expects(self::once())
    }

    public function testDeleteSuccessMock(){
        $product = new Product();
        $product->setId("1");
        $this->repository->expects(self::once())
            ->method("delete")
            ->with(self::equalTo($product));//equalTo() digunakan untuk pengecekan parameter,WAJIB!!

        $this->repository->expects($this->once())
            ->method("findById")
            ->willReturn($product)
            ->with(self::equalTo("1")); //1 karna $product->setId("1");

        $this->service->delete("1");
        self::assertTrue(true, "Success Delete");
    }

    public function testDeleteExceptionMock(){
        $this->expectException(\Exception::class);
        $this->repository->expects($this->once())
        ->method("findById")
        ->willReturn(null)
        ->with(self::equalTo("1")); //equalTo() digunakan untuk pengecekan parameter,WAJIB!!, //1 karna $this->service->delete("1"); 

        $this->service->delete("1"); 

        // $this->repository->delete($product); error karna dipanggilnya 1x expects(self::once()) / cek ProductService
